...
public class QuantityExtractor {
    public static int extractQuantity(String productName) {
        // 使用正则表达式匹配商品名称中的单量
        Pattern pattern = Pattern.compile("(\\d+(\\.\\d+)?)(ml|ML|l|L|升|毫升)");
        Matcher matcher = pattern.matcher(productName);

        if (matcher.find()) {
            // 提取匹配到的数字部分，并转换为整数
            String quantityString = matcher.group().replaceAll("[^\\d]", "");
    ...
    public void testExtractQuantity() {
        ...
        for (int i = 0; i < productNames.length; i++) {
            int quantity = extractQuantity(productNames[i]);
            System.out.println(quantity);
            //Assertions.assertEquals(expectedQuantities[i], quantity);
        }
    }
}
